<template>
  <div class="relative flex flex-col justify-center overflow-hidden py-6 sm:py-12">
    <div @click="this.$router.push('/dashboard/taskblock/'+title.id)" class="w-96 group relative cursor-pointer overflow-hidden bg-white px-6 pt-10 pb-8 shadow-xl ring-1 ring-gray-900/5 transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl sm:mx-auto sm:rounded-lg sm:px-10">
      ...
      <div class="relative z-10 mx-auto">
        <span class="grid h-20 w-20 place-items-center rounded-full bg-green-500 transition-all duration-300 group-hover:bg-green-400">
          <CubeIcon class="text-white w-6 h-6"/>
        </span>
        <div class="space-y-6 pt-5 text-base leading-7 text-gray-600 transition-all duration-300 group-hover:text-white/90">
          <p class="break-words">TaskBlock Name: {{title.name}} </p>
        </div>
        <div class="pt-5 text-base font-semibold leading-7">
          <p>
            <a href="#" class="text-green-500 transition-all duration-300 group-hover:text-white">+ Add a Task &rarr;</a>
          </p>
        </div>
      </div>
    </div>
